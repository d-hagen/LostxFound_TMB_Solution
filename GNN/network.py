...
def transfer_aware_distances(source, transfer_penalty=1.0, step_cost=1.0):
    """Shortest weighted distance from source to every station.

    step_cost          : cost to move one stop along the same line
    transfer_penalty   : additional cost when an edge crosses a line boundary

    Default (1.0, 1.0) yields the passenger-perceived E[hops] metric: 1 per stop,
    +1 per transfer. Setting step_cost low (e.g. 0.05) yields an operator
    movement-cost metric where carrying an item along a line is nearly free
    but transfers are expensive.
    ...
    while heap:
        ...
        for nbr, edge_line in _ADJ.get(stn, set()):
            # Same line → step_cost, different line → step_cost + transfer_penalty
            cost = step_cost if edge_line == cur_line else step_cost + transfer_penalty
            nd = d + cost
